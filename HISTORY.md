...
- 2026-07-03, 00:14: **Sửa lỗi không tải được file `.xdw` từ Google Drive (`SyncManager`)**:
  ...
  - **Xử lý triệt để lỗi "Trôi thời gian" trên Android & FAT32**:
    - **Vấn đề 1**: Khắc phục tình trạng khi tải file từ Google Drive về, hệ điều hành Android tự động gán `lastModified` bằng "thời gian hiện tại lúc tải" khiến file trên điện thoại luôn bị hiểu nhầm là "mới hơn" bản gốc trên mây. Đã bổ sung cơ chế khôi phục chính xác mốc thời gian gốc (ép `setLastModified` bằng với mốc `remoteEpoch` của Drive) ngay sau khi file tải xong.
    - **Vấn đề 2**: Nới rộng khoảng dung sai (tolerance) khi so sánh thời gian từ `500ms` lên `2500ms` (2.5 giây). Điều này xử lý triệt để sai số do đặc thù phân vùng bộ nhớ ngoài định dạng FAT32/exFAT trên các thiết bị Kindle Fire (chỉ lưu vết thời gian theo bội số của 2 giây).
## [v1.5.1] - 2026-07-02
