...
- 2026-07-02: **Tùy chọn "Luôn giữ sáng màn hình khi đọc" (`SettingsActivity` & `PdfViewerActivity`)**:
...
- 2026-07-02: **Làm nổi bật và tùy chỉnh màu sắc thông tin `自社品番, 品番, 自社品名, 品名` (`SettingsActivity`, `SettingsManager`, `PdfMetadataManager`)**:
...
- 2026-07-02: **`UpdateChecker` & `UpdateCheckerWithProgress` — Cải tiến kiểm tra, lưu file và mở thư mục Download**:
  ...
  - **Tự động mở trình quản lý file tại thư mục Download khi tải về hoàn tất**: Theo yêu cầu mới, ngay sau khi tải bản cập nhật xong, ứng dụng sẽ tự động mở thẳng Trình quản lý file / Quản lý tải xuống của hệ thống hướng đúng vào thư mục `Download` (sử dụng `DownloadManager.ACTION_VIEW_DOWNLOADS` kết hợp fallback URI `resource/folder`), đồng thời hiện thông báo nhắc người dùng bấm vào file `MyPDFReader-update.apk` để cài đặt. Giải pháp này giúp vượt qua mọi rào cản bảo mật của Android 8+ khi cài ứng dụng từ bên thứ ba.
...
- 2026-07-02: **`MainActivity` & `PdfFileAdapter` — Số thứ tự không tự cập nhật khi sửa**: 
